...
def test_agent_request_has_independent_scheduler_state(tmp_path):
    ...
    state.begin_next_autoresearch_action(
        {"kind": "select_frontier", "allow_agent_request": True}
    )

    requested = state.request_manager_agent_action(_request())
    assert "parent_sha" not in requested
...
def test_frontier_selection_binds_scheduled_agent_after_selecting_parent(tmp_path):
    ...
    state.record_next_autoresearch_action_decision(
        "select_frontier",
        {"node_sha": checkpoint.sha, "reason": "Continue this direction."},
    )

    result = _manager(tmp_path, state).select_frontier_for_next_proposal(
        on_select=lambda selection: selection,
        allow_agent_request=True,
    )

    assert result["selected_frontier_node_sha"] == checkpoint.sha
    action = HitlRuntimeState(tmp_path).manager_agent_action()
    assert action["parent_sha"] == checkpoint.sha


def test_manager_request_availability_is_limited_to_eligible_selection(tmp_path):
    state = HitlRuntimeState(tmp_path)
    state.begin_next_autoresearch_action(
        {"kind": "select_frontier", "allow_agent_request": True}
    )
    ...
    response = manager.request_agent_run(
        "resource_finder",
        "Find an external validation dataset.",
        "The retained result has only synthetic evaluation.",
    )
    assert response.startswith("Runtime attached resource_finder")
...
def test_manager_request_is_hidden_when_no_further_proposal_will_run(tmp_path):
    state = HitlRuntimeState(tmp_path)
    state.begin_next_autoresearch_action(
        {"kind": "select_frontier", "allow_agent_request": False}
    )
    manager = _manager(tmp_path, state)
...
def test_next_request_keeps_prior_context_for_same_frontier(tmp_path):
    state = HitlRuntimeState(tmp_path)
    state.begin_next_autoresearch_action(
        {"kind": "select_frontier", "allow_agent_request": True}
    )
    ...
    assert second["base_parent_sha"] == "parent"
    assert second["base_context_sha"] == "context-1"
