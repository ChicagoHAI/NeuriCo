...
def test_sealed_dataset_stages_into_test_dir(tmp_path):
    ...
    # The sealed dataset is staged as plaintext at the eval-facing data/.test
    # path; the scoring seal relocates it out of the workspace while agents run.
...
def test_sealed_in_repo_resource_is_moved_not_left_readable(tmp_path):
    ...
    # Staged into data/.test; the readable in-repo copy is removed
    assert (work_dir / SEALED_STAGING_DIR / "benchmark.csv").exists()
...
def test_sealed_source_inside_source_repo_removes_adopted_copy(tmp_path):
    ...
    staged = stage_local_resources(work_dir, idea_spec)
    assert staged == 1
    assert (work_dir / SEALED_STAGING_DIR / "bench.csv").exists()
...
def test_sealing_whole_source_repo_never_deletes_workspace(tmp_path):
    ...
    assert (work_dir / SEALED_STAGING_DIR / "repo" / "bench.csv").exists()
...
def test_scoring_seal_relocates_sealed_data_out_of_workspace(tmp_path):
    # The protection mechanism: sealed data staged at data/.test is moved out
    # of the workspace to .scoring_sealed while agents run, and restored on
    # unseal. So an agent phase never sees it at data/.test.
    from core.scoring_seal import (seal_scoring_files, unseal_scoring_files,
                                    sealed_dir_for)
    work_dir = tmp_path / "workspaces" / "ws"
    (work_dir / SEALED_STAGING_DIR).mkdir(parents=True)
    bench = work_dir / SEALED_STAGING_DIR / "bench.csv"
    bench.write_text("x,label\n1,0\n")
    (work_dir / "scoring").mkdir()
    (work_dir / "scoring" / "eval.py").write_text("print('x')\n")
    (work_dir / "scoring" / "targets.json").write_text("{}")

    sealed_dir = seal_scoring_files(work_dir)
    # Sealed away: gone from the workspace, present under .scoring_sealed.
    assert not bench.exists()
    assert (sealed_dir_for(work_dir) / SEALED_STAGING_DIR / "bench.csv").exists()
    unseal_scoring_files(work_dir, sealed_dir)
    assert bench.read_text() == "x,label\n1,0\n"


def test_isolated_scorer_reads_sealed_data_from_frozen_copy(tmp_path):
    # End to end through make_isolated_continuation_scorer: sealed data is
    # staged plaintext at data/.test, sealed out of the workspace, and copied
    # into the frozen scorer worktree where eval.py reads it. No agent runs
    ...
    # Sealed held-out data staged as plaintext, as staging would leave it.
    (work_dir / SEALED_STAGING_DIR).mkdir(parents=True)
    (work_dir / SEALED_STAGING_DIR / "bench.csv").write_text("x,label\n1,0\n2,1\n")
    ...
    # The results review copy comes back to the live workspace.
