...
Shared building blocks for the CLIs that convert free-form idea sources
(IdeaHub pages, local idea files, continuation intention files) into NeuriCo
idea YAML:
...
- The extraction rules for local resources and evaluation specs, embedded
  verbatim in each converter's prompt so every entry point applies the same
  contract semantics
"""
...
def save_yaml_file(result: dict, source_ref: str, author: str = None, *,
    ...
    Shared by the converter CLIs, which differ only in provenance: the
