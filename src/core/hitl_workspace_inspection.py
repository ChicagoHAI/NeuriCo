...
class HitlWorkspaceInspector:
    ...
    def read_workspace_file(self, path: str, offset: int = 1, limit: int = 200) -> str:
        ...
        selected, total_lines, truncated_by_bytes = self._read_line_range(
            target,
        ...
        return self._render(
            {
                ...
                or normalized_offset - 1 + normalized_limit < total_lines,
                "content": numbered,
            }
        )
    ...
    def _read_line_range(
        ...
        """Return a bounded line range while counting the complete file exactly."""
        selected: list[str] = []
        selected_bytes = 0
        ...
        with path.open("rb") as handle:
            for raw_line in handle:
                ...
                if b"\x00" in raw_line:
                    raise HitlWorkspaceInspectionError("binary files cannot be read or searched.")
                if total_lines < offset or total_lines > end:
                    continue
                if selection_truncated or selected_bytes + len(raw_line) > _MAX_READ_BYTES:
                    truncated_by_bytes = True
                    selection_truncated = True
                    continue
                try:
                    selected.append(raw_line.decode("utf-8").rstrip("\r\n"))
                except UnicodeDecodeError as exc:
                    raise HitlWorkspaceInspectionError(
                        "only UTF-8 text files can be read or searched."
                    ) from exc
        ...
        return selected, total_lines, truncated_by_bytes
