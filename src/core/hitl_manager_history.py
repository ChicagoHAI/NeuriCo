...
class HitlManagerHistory:
            ...
            with self._connect() as connection:
                connection.executescript("""
                    ...
                    CREATE TABLE IF NOT EXISTS conversation_records (
                        ...
                        visible_to_human INTEGER NOT NULL DEFAULT 0
                    );
                ...
                if "visible_to_human" not in columns:
                    connection.execute(
                        "ALTER TABLE conversation_records "
                        "ADD COLUMN visible_to_human INTEGER NOT NULL DEFAULT 0"
                    )
                    self._migrate_legacy_visible_messages(connection)
                try:
                    connection.execute(
    ...
    def append(self, record: Dict[str, object]) -> None:
        ...
        with self._lock():
            with self._connect() as connection:
                visible = bool(record.get("visible_to_human", record_type == "message" and speaker == "human"))
                inserted = connection.execute(
                    "INSERT OR IGNORE INTO conversation_records "
                    "(record_id, record_type, speaker, content, created_at, visible_to_human) "
                    "VALUES (?, ?, ?, ?, ?, ?)",
                    (record_id, record_type, speaker, content, created_at, int(visible)),
                )
    ...
    @staticmethod
    def _migrate_legacy_visible_messages(connection: sqlite3.Connection) -> None:
        """Conservatively recover direct messages from pre-visibility archives.

        Older archives did not distinguish a manager's private ReAct text from
        text sent through a human-facing tool.  Human input is always visible;
        manager text is restored only when it immediately follows the explicit
        ask/answer tool that delivered it to the human.
        """
        connection.execute(
            "UPDATE conversation_records SET visible_to_human = 1 "
            "WHERE record_type = 'message' AND speaker = 'human'"
        )
        connection.execute(
            """
            UPDATE conversation_records AS message
            SET visible_to_human = 1
            WHERE message.record_type = 'message' AND message.speaker = 'manager'
              AND EXISTS (
                SELECT 1 FROM conversation_records AS call
                WHERE call.sequence = message.sequence - 1
                  AND call.record_type = 'function_call'
                  AND (call.content LIKE 'Tool call ask_human:%'
                       OR call.content LIKE 'Tool call answer_to_human:%')
              )
            """
        )

    def visible_messages(self) -> List[Dict[str, str]]:
        """Return only the durable human/manager conversation shown in the UI."""
        with self._lock():
            with self._connect() as connection:
                rows = connection.execute(
                    """
                    SELECT sequence, record_id, speaker, content, created_at
                    FROM conversation_records
                    WHERE record_type = 'message' AND visible_to_human = 1
                      AND speaker IN ('human', 'manager')
                    ORDER BY sequence
                    """
                ).fetchall()
        return [dict(row) for row in rows]
