...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for raw evidence ingestion."""
    parser = argparse.ArgumentParser(
        description="Ingest raw evidence from historical NeuriCo repositories into SQLite."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory whose immediate child directories are research-run repos.",
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Only ingest this repository name. Repeatable.",
    )

    parser.add_argument(
        "--parser-version",
        default=PARSER_VERSION,
        help="Parser version recorded on parsed files.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Continue after one repository fails. Enabled by default.",
    )
...
def build_source_file_record(
    ...
    *,
    ...
    """Convert scanner metadata into SourceFileRecord."""
    return SourceFileRecord.create(
        ...
        parser_version=parser_version if discovered.is_structured else None,
    )
...
def persist_parse_result(
    *,
    ...
    """
    Store parsed records and parse errors for one source file.

    Returns:
        (records_written, errors_written)  
    """
    records_written = 0
    errors_written = 0
    ...
    for parsed_record in result.records:
        ...
        store.insert_raw_record(connection, record)
        records_written += 1
    ...
    for parsed_error in result.errors:
        ...
        store.insert_parse_error(connection, error)
        errors_written += 1
...
def ingest_repository(
    *,
    ...
    """Ingest one repository in one transaction."""
    summary = {
        "files_discovered": 0,
        "structured_files": 0,
        "unparsed_files": 0,
    ...
    with store.transaction() as connection:
        store.upsert_run(connection, run_record)
        ...
        for discovered in iter_repository_files(repository):
            ...
            try:
                result = parse_structured_file(discovered.absolute_path)
...
def merge_counts(
    ...
    """Add repo summary counts into the ingestion summary."""  
    for key, value in source.items():
        target[key] = target.get(key, 0) + value


def run_ingestion(args: argparse.Namespace) -> int:
    """Run raw evidence ingestion from CLI args."""
    ...
    with store.transaction() as connection:
        store.upsert_ingestion_run(connection, ingestion)
    ...
    for index, repository in enumerate(repositories, start=1):
        print(f"[{index}/{len(repositories)}] Ingesting {repository.repo_name}")  
        ...
        try:
            ...
            print(
                f"[error] {repository.repo_name}: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
        ...
        merge_counts(totals, repository_summary)
