...
def _payload(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.idea_type == "evidence":
        ...
        if not args.option:
            raise HitlCommandUsageError("decision ideas require at least one --option.")
...
def main(argv: List[str] | None = None) -> int:
    ...
    try:
        ...
        return command_error("hitl-report-idea", str(exc), EXAMPLE)
    except HitlToolClientError as exc:
        return fail("hitl-report-idea", exc)
